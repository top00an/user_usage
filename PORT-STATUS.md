...
### 10. 프런트 UI 부채 — 2026-08-14 감사에서 **실측으로 확정**, 반경이 넓어 남긴 것

읽기 전용 UI 감사 2건(폼·컨트롤 일관성 / 스타일 시스템)의 결과 중 19건은 같은 커밋에 착지했고,
아래 셋은 남겼습니다. **권장 순서는 10-b → 팔레트 통합 → 10-a 입니다** — 팔레트가 두 벌인 상태로
테마화하면 *한 벌만 테마를 타는 상태*가 되고, 그건 지금보다 나쁩니다("테마 대응이 됐다"고 믿게 되니까).

**10-a. ECharts 가 다크 전용입니다.** `web/components/grafana/options.ts` 의 축·범례·값 라벨·툴팁
색이 상수로 박혀 있습니다. 라이트 테마 대비(계산값): 막대 값 라벨 **1.41:1**, 축 라벨 **3.13:1**.
사람별 비용을 다루는 화면에서 값이 "흐리게 보이는" 것이 아니라 **0으로 읽힐** 자리입니다.
캔버스 렌더러는 CSS 변수를 읽지 못하므로 **토큰화만으로는 안 고쳐집니다** — 옵션 빌더 4개
(area·donut·bar·gauge)를 인자화하고, `web/components/charts/EChart.tsx` 가 테마를 구독해
`setOption` 을 다시 타야 합니다. 호출부 11곳이 지나가는 경로라 라이트/다크 실측 스크린샷 없이는
검증이 성립하지 않습니다.

**10-b. 죽은 코드 ~170줄(`globals.css` 의 10%)과 `web/components/charts.tsx` 전체.**
프로덕션 임포터가 0인데 `web/test/grafana-layout.test.tsx` 의 `Donut` 임포트 하나가 초록불로
유지하고 있습니다. 하필 그 죽은 쪽에만 **CVD 검증 팔레트 토큰(`--series-*`)을 읽는 코드**가
있어서, 지금 차트 색 체계가 두 벌입니다(토큰·테마대응=죽음 / 하드코딩·다크전용=살아있음).
지울 때는 그 테스트도 함께 지웁니다 — 테스트만 남기면 죽은 코드가 다시 굳습니다. 이걸 먼저
치우면 `@media` 중단점 10종이 **저절로 5종으로** 정리됩니다(겹치는 넷이 죽은 셀렉터에만 붙어 있음).

**10-c. 자체 기준이 인라인 style 로만 삽니다.** 되돌릴 수 없는 버튼의 터치 타깃 `minHeight:44`
(WCAG 2.2 요구치는 24×24 이고 44 는 우리 기준입니다)가 `web/components/admin/UserSheet.tsx`·
`web/components/admin/RevokeCell.tsx` 3곳, 오류 톤 카드 강조가 2곳입니다. 규칙이 아니라 예외로
남아 있어 **다음 파괴 버튼은 그냥 39.7px 로 만들어집니다.** 새 토큰(`--touch-min`)·새 클래스
(`.card.danger`) 신설은 계약 변경이라 단독으로 하지 않았고, 지금은 왜 그 값인지 주석만 남겼습니다.

> 함께 확인된 사실: `web/app/globals.css` 의 `@media (max-width: 720px)` 와
> `web/lib/dashLayout.ts` 의 `STACK_MAX_W` 는 같은 값이어야 하는데 **그 쌍을 지키는 테스트가
> 없습니다.** 현재 값은 일치합니다(깨져 있지 않습니다) — 값을 옮기는 사람을 위한 안전망이 없다는 뜻입니다.
