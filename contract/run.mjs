 ...
 * 계약 검증 러너 — 회귀 게이트.
 *
 *   node contract/run.mjs verify --base URL    이미 떠 있는 서버(=Go)를 시드·캡처 → 골든과 대조
 *
 * 골든(contract/golden/)은 2026-08-09 Node 제거 시점에 **동결**됐다. 캡처(골든 재생성)는
 * 현행 Node 서버가 하던 것이라 함께 사라졌고, verify 만 남는다 — verify 는 서버를 스폰하지
 * 않고 **이미 떠 있는 Go 서버**(--base)에 시드·조회해 동결 골든과 대조한다.
 *
...
async function main() {
  ...
  if (cmd === 'verify') {
    ...
    if (!existsSync(GOLDEN)) { console.error('골든이 없다(contract/golden/) — 동결 골든이 있어야 대조한다.'); return 2; }
  ...
  console.error(`알 수 없는 명령: ${cmd} (verify)`);
  return 2;
}
