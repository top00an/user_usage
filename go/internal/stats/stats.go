...
//
// 왜 필요한가: 사용량 화면이 합계만 보여주면 "평균적인 세션"과
...
// 골든이 JSON 숫자를 그대로 비교하므로 보간식을 Node 구현(골든의 출처)에서 **그대로** 옮겼다.
...
// Summary 의 JSON 태그는 골든 응답의 키를 그대로 쓴다.
...
func summarizeClean(clean []float64, dropped int, ps []float64) Summary {
	...
	// 합은 **정렬한 뒤** 앞에서부터 — Node 구현(골든의 출처)이 clean.sort() 다음에 reduce 한다.
