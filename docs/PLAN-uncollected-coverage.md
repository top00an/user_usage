...
| Codex `slash` | 미수집 | **BLOCKED 확정 — 스키마에 명령명 자리가 없다(세션 11개·user_message 33건 전수)** |
...
- [x] `git status --short`로 기존 사용자 변경을 기록한다.
- [x] 설치된 각 CLI의 버전과 **실제 데이터 루트**를 기록한다. (2026-08-21: `codex-cli 0.144.4`)
- [x] 현재 테스트를 먼저 실행하고 기준 결과를 저장한다.

> ### ⚠ "실제 데이터 루트"가 이 웨이브에서 가장 값나가는 항목이다
>
> 홈이 **옮겨져 있을 수 있다.** 실측(2026-08-21): 이 머신은 `CODEX_HOME=~/.local/share/orca/
> codex-runtime-home/home` 으로 돌고 있었고, 수집기는 그 변수를 보지 않아 `~/.codex` 의 8개만
> 올렸다. 거부도 경고도 없다 — "세션 8개 전송 완료"라고 말한다. 그러면 화면은 그 사람이
> Codex 를 덜 썼다고 **단정**한다.
>
> 두 홈은 8개를 **하드링크로 공유**하고 있었고 옮긴 홈에만 3개가 있었다(inode 대조). 즉 실제
> 누락은 3세션이다 — 적은 수지만 **경고 없이 사라지는 부류**라 수치보다 성질이 문제다.
>
> 그래서 W0 은 경로를 **가정하지 말고 환경에서 읽어** 기록한다:
>
> ```bash
> env | grep -E '^(CODEX_HOME|CLAUDE_CONFIG_DIR|GEMINI_HOME_DIR|ANTIGRAVITY_HOME_DIR)='
> # 각 홈 아래 세션 파일 수를 세어 수집기 로그의 개수와 대조한다
> ```
>
> 수집기는 이제 도구 자신이 쓰는 변수를 존중한다(`CODEX_HOME` · `CLAUDE_CONFIG_DIR`).
> Gemini·Antigravity 는 그 도구가 어떤 변수로 홈을 옮기는지 **미확인**이다 — 그 원천을 다룰
> 때(§W2·§W4) 함께 확인할 것.
>
> ### ⚠ 「수집됨」이라 적힌 축도 실측으로 재확인한다
>
> §0 표가 Codex `bash` 를 수집됨으로 적고 있었지만 현행 Codex 에서 실제로는 **0건**이었다
> (이벤트 모양이 `custom_tool_call`/`exec` 로 바뀌어 파서가 침묵했다 — 2026-08-21 수정).
> 낡은 「수집됨」이 「미수집」보다 위험하다.
...
**BLOCKED — 원천에 구조화된 필드가 없다. 미수집을 유지한다.**

세션 **11개** · `event_msg/user_message` **33건** 전수를 훑었다(Codex 0.144.4):

- `slash` · `command_name` · `custom_prompt` · `prompt_name` · `builtin` 계열 키 **0건**
- `user_message` 의 키는 `type`·`message`·`images`·`local_images`·`text_elements` **다섯 개뿐**이다 —
  명령명을 담을 자리가 스키마에 **없다**
- `history.jsonl`(11행, 키 `session_id`·`text`·`ts`)에도 슬래시로 시작하는 항목이 **0건**
- 슬래시를 실제로 보낸 턴(`codex exec "/status"`)은 `user_message.message = "/status"` 로만
  남았다 — **프롬프트 원문**이지 구조화된 명령이 아니다

앞선 잠정 판정의 유보 사유("표본에 슬래시가 없었다")가 해소됐다. 슬래시를 보낸 턴이 표본에
들어왔고, 그 턴조차 명령으로 기록되지 않았다.

`message` 가 `/` 로 시작하는지 보고 추출할 수는 있지만 **하지 않는다.** 그건 사용자 프롬프트
전문을 근거로 삼는 방식이고, 이 문서 §W4 가 금지한 것이다. 또 `exec` 경로에서 `/status` 는
슬래시 명령이 아니라 그냥 프롬프트였다(모델이 텍스트로 해석해 git 을 돌렸다) — 즉 `/` 접두는
"슬래시 명령을 썼다"의 근거가 되지 못한다.

재판정 조건: Codex 버전이 올라가 `user_message` 에 명령명 필드가 생기거나, app-server·공식
lifecycle event 가 명령명을 별도로 제공할 때.
