...
**무엇을 증명했나 — 우리 배선.** 목 세션 6개(`oss`·`ollama`·`lmstudio`·`openai`·`together`·
provider 미기록)를 만들어 **실제 수집기 바이너리**로 두 번 돌렸다:

| 케이스 | config 블록 | 기대 | 결과 |
|---|---|---|---|
| `oss` | 없음 | local (이름 폴백) | ✅ |
| `ollama` | 없음 | local (이름 폴백) | ✅ |
| `lmstudio` | 없음 | local (이름 폴백) | ✅ |
| `ollama` | **공인 주소** | (없음) — config 승 | ✅ |
| `together` | **사설 대역** | local — config 승 | ✅ |
| `openai` | 없음 | (없음) — 기본 provider | ✅ |
| 미기록 | — | (없음) | ✅ |

그리고 **드라이런이 아니라 실제로 전송**해 서버까지 왕복했다: 세션 6건 저장 →
`?runtime=local` 3행 · `?runtime=cloud` 3행 · 미지정 6행 · `?runtime=locl` → 400.
`summary`·`series`·`distribution`·`leaderboard`·`quality` 도 3/6 으로 좁혀졌다(자식 표를 세션으로
되짚는 EXISTS 조건이 실제로 걸린다는 뜻이다). 로컬 모델 이름은 미등록으로 빠졌다
(`qwen3-coder:30b` 등이 `unpriced` 에 오른다).

**무엇을 증명하지 못했나 — 원천의 형식.** 목데이터는 `model_provider` 값을 **우리가 지어낸
것**이다. 그래서 "Codex 가 `--oss` 에서 실제로 그 이름을 쓴다"는 것은 여전히 미확인이고,
목데이터로는 확인될 수 없다. 이 문서 §5 의 규칙("샘플 없으면 parser 를 쓰지 않는다")이
말하는 샘플은 **원천이 만든 것**이지 우리가 만든 것이 아니다. 목데이터가 답하는 질문은
"형식이 이러할 때 우리 코드가 옳게 판정하나"이고, 그 답이 위 표다.
