...
> ### ⚠ 수집기가 훑는 홈이 옮겨져 있을 수 있습니다
>
> Codex 는 `CODEX_HOME`, Claude Code 는 `CLAUDE_CONFIG_DIR` 로 자기 홈을 옮깁니다. 옮기는 도구가
> 실제로 있습니다(실측 2026-08-21: Orca 가 `CODEX_HOME` 을 옮겨 띄웁니다). 수집기는 이제 두 변수를
> 존중하고 **Codex 는 옮긴 홈과 기본 홈을 둘 다** 훑습니다 — 같은 PC 에서 다른 터미널로도 쓰면
> 세션이 두 곳에 나뉘기 때문입니다.
>
> 수집이 적어 보이면 로그의 디렉터리와 세션 수를 직접 대조하세요:
>
> ```sh
> env | grep -E '^(CODEX_HOME|CLAUDE_CONFIG_DIR)='
> usage-collector -platform=codex -dry-run -all -state=/tmp/s.json 2>&1 | grep '^\[codex\]'
> # → 훑은 디렉터리와 세션 수가 줄마다 나옵니다. 아는 홈이 빠져 있으면 그것이 원인입니다.
> ```
>
> 실측 사례(2026-08-21): 이 변수를 안 볼 때 훑은 세션이 `8개`, 볼 때 `19개`였습니다
> (옮긴 홈 11 + 기본 홈 8). **저장된 세션은 3개 늘었습니다** — 두 홈이 8개를 하드링크로
> 공유하고 있어 그만큼은 같은 파일이었고, 옮긴 홈에만 있던 것이 3개였습니다.
> 중복은 문제되지 않습니다(세션 id 절대값 UPSERT). 요점은 **한쪽만 보면 그 3개가 통째로
> 빠지고 아무 경고도 없다**는 것입니다.
